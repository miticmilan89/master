 ...
 * 
 * <p>
 * Message arguments can be specified via the {@link #setArguments(Object) arguments} attribute or by using nested {@code <spring:argument>} tags.
 * 
 * @author Rod Johnson
...
public class MessageTag extends HtmlEscapingAwareTag implements ArgumentAware {
	...
	private transient MessageSourceResolvable message;
	...
	private transient List<Object> nestedArguments;
	...
	public int doEndTag() throws JspException {
		try {
			...
			if (msg != null)
				msg = StringUtils.replace(msg, "_TR", "");
	...
	protected String resolveMessage() throws JspException {
		...
		if (this.text != null) {
			...
			if (ContextHolder.isLoggedUserParticipant()) {
				// support for overriding labels for specified banks
				LoggedParticipantData loggedUserParticpant = ContextHolder.getLoggedParticipantData();
				String key = loggedUserParticpant.getParticipantId() + ".".concat(this.code);
				String s = messageSource.getMessage(key, argumentsArray, this.text, getRequestContext().getLocale());
		...
		if (ContextHolder.isLoggedUserParticipant()) {
			...
			LoggedParticipantData loggedUserParticpant = ContextHolder.getLoggedParticipantData();
			String key = loggedUserParticpant.getParticipantId() + ".".concat(this.code);
			String s = messageSource.getMessage(key, argumentsArray, getRequestContext().getLocale());
			if (s != null && !s.equals(key))
				return s;
		}
	...
	private Object[] appendArguments(Object[] sourceArguments, Object[] additionalArguments) {
		...
		Object[] argumentList = new Object[sourceArguments.length + additionalArguments.length];
		System.arraycopy(sourceArguments, 0, argumentList, 0, sourceArguments.length);
		System.arraycopy(additionalArguments, 0, argumentList, sourceArguments.length, additionalArguments.length);
		return argumentList;
	}
	 ...
	 * @see #setArguments
	 */
	protected Object[] resolveArguments(Object argumentList) throws JspException {
		if (argumentList instanceof String) {
			String[] stringArray = StringUtils.delimitedListToStringArray((String) argumentList, this.argumentSeparator);
			...
			return stringArray;
		} else if (argumentList instanceof Object[]) {
			return (Object[]) argumentList;
		} else if (argumentList instanceof Collection) {
			return ((Collection<?>) argumentList).toArray();
		} else if (argumentList != null) {
			// Assume a single argument object.
			return new Object[] { argumentList };
		} else {
			return Constants.getEmptyObjectArray();
		}
	}
