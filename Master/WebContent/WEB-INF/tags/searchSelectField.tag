...
<%@ attribute name="items" required="false" rtexprvalue="true" type="java.util.List" description="Items with pair of value and label" %>
<%@ attribute name="itemsMap" required="false" rtexprvalue="true" type="java.util.HashMap" description="Items with pair of value and label" %>
...
<%@ attribute name="htmlEscape" required="false" rtexprvalue="true" description="Css class"  %>
	...
	<c:if test="${!norow}">
        ...
        <div class="column-data  ${dataClass}" <c:if test="${!norow}">style="width:${pageScope._tstRW }"</c:if>>
            <c:choose>
        		...
        		<c:otherwise>
		            <form:select path="${name}" size="${size}" cssStyle="selectW" onchange="${onchange}" cssClass="${fieldClass }" >
		            	<c:forEach items="${items}" var="item">
		            		...
		            		<form:option value="${item.value}" label="${_label}" htmlEscape="${htmlEscape==false ? 'false' : 'true' }"/>
		            	</c:forEach>
		            		<c:remove var="_label"/>
		            </form:select>
          		</c:otherwise>
